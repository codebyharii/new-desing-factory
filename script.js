document.addEventListener('DOMContentLoaded', () => {
        ...
        readMoreBtns.forEach(btn => {
            btn.addEventListener('click', (e) => {
                ...
                if (isMobile) {
                    ...
                    gsap.set(detailBg, { 
                        width: '80%', height: '65%', top: '50%', left: '50%', 
    ...
    pwTimeline.from([".pw-subtitle", ".pw-title"], 
        { y: "100%", opacity: 0, duration: 0.8, stagger: 0.1, ease: "power4.out" }
    )
    // 2. Image Clip-Path & Parallax Scale
    .from(".pw-image-box",
        { clipPath: "inset(100% 0% 0% 0%)", opacity: 0, duration: 1.2, ease: "power3.inOut" },
        "-=0.4" // Overlap slightly with text reveal
    )
    .from(".pw-main-img",
        { scale: 1.4, duration: 1.8, ease: "power2.out" },
        "-=1.2" // Start scaling as clip-path opens
    )
    // 3. Elastic Details Stagger
    .from(".pw-details-box > *",
        { x: 50, opacity: 0, duration: 0.8, stagger: 0.1, ease: "back.out(1.4)" },
